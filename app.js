...
function drawDefinition() {
    ...
    ctx.fillText(`sin(${currentAngle}°) = ${sinVal.toFixed(2)}`, G_ORIGIN_X + 60, CY + R_BASE + 60);
    ctx.fillStyle = activeColors['cos'];
    ctx.fillText(`cos(${currentAngle}°) = ${cosVal.toFixed(2)}`, G_ORIGIN_X + 60, CY + R_BASE + 95);
    ctx.fillStyle = activeColors['tan'];
    ctx.fillText(`tan(${currentAngle}°) = ${tanStr}`, G_ORIGIN_X + 60, CY + R_BASE + 130);
}
